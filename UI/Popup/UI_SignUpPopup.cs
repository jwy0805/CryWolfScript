...
public class UI_SignUpPopup : UI_Popup
{
    ...
    private async void OnSignUpClicked(PointerEventData data)
    {
        ...
        else
        {
            ...
            if (response.CreateOk == false) return;
            
            var createInitDeckPacket = new CreateInitDeckPacketRequired { UserAccount = account };
            var deckResponse = await Managers.Web.SendPostRequestAsync<CreateInitDeckPacketResponse>(
                "Collection/CreateDeck", createInitDeckPacket);
            if (deckResponse.CreateDeckOk) return;
            Debug.LogError("유저 정보 초기화 오류");
        }
    }
